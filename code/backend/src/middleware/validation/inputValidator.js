...
/**
 * Enhanced Input Validation Middleware for LendSmart
...
class InputValidator {
  ...
  get userRegistrationSchema() {
    return this.joi.object({
      ...
      password: this.joi.string().pattern(this.patterns.strongPassword).required()
        .messages({
      ...
      confirmPassword: this.joi.string().valid(this.joi.ref('password')).required()
        .messages({
      ...
      address: this.joi.object({
        ...
        zipCode: this.joi.string().pattern(/^\d{5}(-\d{4})?$/).required(),
        country: this.joi.string().trim().length(2).default('US')
      }).required(),
      ssn: this.joi.financial().ssn().required(),
      employmentStatus: this.joi.string().valid('employed', 'self_employed', 'unemployed', 'retired', 'student').required(),
  ...
  get userLoginSchema() {
    return this.joi.object({
      ...
      mfaCode: this.joi.string().pattern(/^\d{6}$/).when('mfaRequired', {
        is: true,
  ...
  get loanApplicationSchema() {
    return this.joi.object({
      ...
      collateral: this.joi.object({
        type: this.joi.string().valid('real_estate', 'vehicle', 'securities', 'other').required(),
        value: this.joi.financial().currency().min(0).required(),
        description: this.joi.string().trim().max(200).required()
      }).optional(),
      employmentInfo: this.joi.object({
  ...
  get paymentSchema() {
    return this.joi.object({
      ...
      paymentMethod: this.joi.string().valid('bank_transfer', 'debit_card', 'credit_card').required(),
      paymentDetails: this.joi.object({
        ...
        cardNumber: this.joi.string().creditCard().when('paymentMethod', {
          is: this.joi.valid('debit_card', 'credit_card'),
          then: this.joi.required(),
          otherwise: this.joi.forbidden()
        }),
        expiryMonth: this.joi.number().integer().min(1).max(12).when('paymentMethod', {
          is: this.joi.valid('debit_card', 'credit_card'),
          then: this.joi.required(),
          otherwise: this.joi.forbidden()
        }),
        expiryYear: this.joi.number().integer().min(new Date().getFullYear()).when('paymentMethod', {
          is: this.joi.valid('debit_card', 'credit_card'),
          then: this.joi.required(),
          otherwise: this.joi.forbidden()
        }),
        cvv: this.joi.string().pattern(/^\d{3,4}$/).when('paymentMethod', {
          is: this.joi.valid('debit_card', 'credit_card'),
          then: this.joi.required(),
          otherwise: this.joi.forbidden()
        })
  ...
  get kycDocumentSchema() {
    return this.joi.object({
      documentType: this.joi.string().valid(
        'passport',
        'drivers_license',
        'national_id',
        'utility_bill',
      ...
      selfieImage: this.joi.string().base64().when('documentType', {
  ...
  get userProfileUpdateSchema() {
    return this.joi.object({
      ...
      employmentStatus: this.joi.string().valid('employed', 'self_employed', 'unemployed', 'retired', 'student').optional(),
  ...
  get passwordChangeSchema() {
    return this.joi.object({
      ...
      newPassword: this.joi.string().pattern(this.patterns.strongPassword).required()
        .messages({
          'string.pattern.base': 'Password must contain at least 8 characters, including uppercase, lowercase, number, and special character'
        }),
      confirmNewPassword: this.joi.string().valid(this.joi.ref('newPassword')).required()
        .messages({
          'any.only': 'Passwords do not match'
        })
    });
  ...
  validate(schema, source = 'body') {
    return (req, res, next) => {
      try {
        ...
        if (source === 'body' && (req.path.includes('auth') || req.path.includes('payment'))) {
