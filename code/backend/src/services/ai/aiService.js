...
class AIService {
  constructor() {
    this.modelEndpoints = {
      riskAssessment:
        process.env.AI_RISK_MODEL_ENDPOINT ||
        "http://localhost:8001/predict/risk",
      creditScoring:
        process.env.AI_CREDIT_MODEL_ENDPOINT ||
        "http://localhost:8002/predict/credit",
      fraudDetection:
        process.env.AI_FRAUD_MODEL_ENDPOINT ||
        "http://localhost:8003/predict/fraud",
    };
  ...
  _processRiskAssessmentResponse(data) {
    if (!data || typeof data.risk_score !== "number") {
      throw new AppError(
        "Invalid risk assessment response format",
        500,
        "AI_RESPONSE_ERROR",
      );
    }
  ...
  _processCreditScoreResponse(data) {
    if (!data || typeof data.credit_score !== "number") {
      throw new AppError(
        "Invalid credit score response format",
        500,
        "AI_RESPONSE_ERROR",
      );
    }
  ...
  _processFraudDetectionResponse(data) {
    if (!data || typeof data.fraud_score !== "number") {
      throw new AppError(
        "Invalid fraud detection response format",
        500,
        "AI_RESPONSE_ERROR",
      );
    }
