...
const performanceFormat = winston.format.combine(
    ...
    winston.format.json(),
    winston.format.printf(
        ({ timestamp, level, message, method, url, duration, statusCode, ...meta }) => {
            const logEntry = {
                timestamp,
                level: level.toUpperCase(),
...
const auditLogger = winston.createLogger({
    ...
    format: winston.format.combine(
        ...
        winston.format.json(),
        winston.format.printf(
            ({ timestamp, level, message, userId, action, resource, before, after, ...meta }) => {
                const logEntry = {
                    timestamp,
                    level: level.toUpperCase(),
                    category: 'AUDIT',
...
const businessLogger = winston.createLogger({
    ...
    format: winston.format.combine(
        ...
        winston.format.json(),
        winston.format.printf(
            ({ timestamp, level, message, loanId, userId, amount, operation, ...meta }) => {
                const logEntry = {
                    timestamp,
                    level: level.toUpperCase(),
                    category: 'BUSINESS',
