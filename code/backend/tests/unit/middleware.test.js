...
describe('Security Middleware', () => {
    ...
    describe('IP Blocking', () => {
        ...
        test('should block access after multiple failed attempts', async () => {
            ...
            const response = await request(app).get('/test').set('X-Forwarded-For', clientIP);
    ...
    describe('Suspicious Activity Detection', () => {
        ...
        test('should block IP after multiple suspicious activities', async () => {
            ...
            for (const query of suspiciousQueries) {
                await request(app).get('/test').query({ search: query });
            }

            // Next request should be blocked
            const response = await request(app).get('/test').query({ search: 'normal query' });
...
describe('Input Validation Middleware', () => {
    ...
    describe('User Registration Validation', () => {
        test('should validate valid user registration data', async () => {
            app.post('/register', inputValidator.validateUserRegistration, (req, res) =>
        ...
        test('should reject weak passwords', async () => {
            ...
            const weakPasswords = ['123456', 'password', 'Password', 'Password123', 'password123!'];
        ...
        test('should reject missing required fields', async () => {
            ...
            const response = await request(app).post('/register').send(incompleteUser).expect(400);
    ...
    describe('Loan Application Validation', () => {
        test('should validate valid loan application', async () => {
            app.post('/loan', inputValidator.validateLoanApplication, (req, res) =>
                res.json({ success: true }),
            );
            ...
            const response = await request(app).post('/loan').send(validLoan).expect(200);
    ...
    describe('Payment Validation', () => {
        test('should validate valid payment data', async () => {
            app.post('/payment', inputValidator.validatePayment, (req, res) =>
                res.json({ success: true }),
            );
            ...
            const response = await request(app).post('/payment').send(validPayment).expect(200);
