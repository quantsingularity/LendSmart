...
async function main() {
    ...
    const loanContract = await LoanContract.deploy(initialOwner, initialFeeRate, initialFeeRecipient);
...
function saveFrontendFiles(contract, contractName) {
    ...
    const mobileContractsDir = path.join(__dirname, "..", "..", "mobile-frontend", "src", "contracts");
