...
contract LoanContract is Ownable, ReentrancyGuard, Pausable {
    ...
    modifier onlyLender(uint256 _loanId) {
        require(
            ...
            "LoanContract: Caller is not the lender"
        );
        _;
    }
    ...
    modifier loanExists(uint256 _loanId) {
        require(
            ...
            "LoanContract: Loan does not exist"
        );
        _;
    }
    ...
    function requestLoan(
        ...
        require(
            _token != address(0),
            "LoanContract: Token address cannot be zero"
        );
        require(
            _principal > 0,
            "LoanContract: Principal must be greater than zero"
        );
        require(
            _duration > 0,
            "LoanContract: Duration must be greater than zero"
        );
    ...
    function fundLoan(
        ...
        require(
            ...
            "LoanContract: Loan not in Requested state"
        );
        require(
            msg.sender != loan.borrower,
            "LoanContract: Borrower cannot fund their own loan"
        );
        ...
        onlyBorrower(_loanId)
    {
        ...
        require(
            loan.status == LoanStatus.Active,
            "LoanContract: Loan not in Active state"
        );
        require(
            _amount > 0,
            "LoanContract: Repayment amount must be greater than zero"
        );
        ...
        if (
            loan.status != LoanStatus.Repaid &&
            block.timestamp > loan.fundedTime + loan.duration
        ) {
