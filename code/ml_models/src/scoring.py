...
class DigitalFootprintScorer(AlternativeDataScorer):
    ...
    def preprocess_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'has_professional_email' in df_data.columns:
            df_data['has_professional_email'] = df_data['has_professional_email'].astype(int)
    ...
    def preprocess_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        for col in tx_data.columns:
            if tx_data[col].dtype == 'object':
                ...
                tx_data[col] = tx_data[col].fillna(tx_data[col].median() if len(tx_data) > 1 else 0)
        ...
        if 'expense_to_income_ratio' in tx_data.columns:
            ...
            tx_data['expense_to_income_ratio'] = tx_data['expense_to_income_ratio'].clip(0, 1)
            # Inverse relationship - lower is better
    ...
    def preprocess_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        for col in up_data.columns:
            if up_data[col].dtype == 'object':
                ...
                up_data[col] = up_data[col].fillna(up_data[col].median() if len(up_data) > 1 else 0)
        
        # Feature transformations
        if 'utility_missed_payments_count' in up_data.columns:
            # Normalize by history length if available
            if 'utility_history_length_months' in up_data.columns and up_data['utility_history_length_months'].iloc[0] > 0:
                history_length = up_data['utility_history_length_months'].iloc[0]
                missed_rate = up_data['utility_missed_payments_count'].iloc[0] / history_length
                # Inverse relationship - lower is better
        ...
        if 'utility_history_length_months' in up_data.columns:
            ...
            up_data['history_length_score'] = np.clip(up_data['utility_history_length_months'] / 36, 0, 1)
    ...
    def score(self, data: pd.DataFrame) -> float:
        ...
        for feature, weight in self.weights.items():
            if feature in up_data.columns:
                ...
                score_features[feature] = value
            elif feature == 'utility_missed_payments_count' and 'missed_payment_rate' in up_data.columns:
                # Use transformed feature instead
                value = up_data['missed_payment_rate'].iloc[0]
                score_features[feature] = value
            elif feature == 'avg_days_late_when_late' and 'days_late_score' in up_data.columns:
                # Use transformed feature instead
                value = up_data['days_late_score'].iloc[0]
                score_features[feature] = value
            elif feature == 'utility_history_length_months' and 'history_length_score' in up_data.columns:
                # Use transformed feature instead
...
class EducationEmploymentScorer(AlternativeDataScorer):
    ...
    def preprocess_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        ee_cols = [col for col in data.columns if col.startswith('education_employment_')]
        ...
        for col in ee_data.columns:
            if ee_data[col].dtype == 'object':
                ...
                ee_data[col] = ee_data[col].fillna(ee_data[col].median() if len(ee_data) > 1 else 0)
        
        # Feature transformations
        if 'employment_years' in ee_data.columns:
            # Normalize to 0-1 scale (0-20 years)
            ee_data['employment_years_score'] = np.clip(ee_data['employment_years'] / 20, 0, 1)
        ...
        if self.model:
            ...
            for feature, weight in self.weights.items():
                if feature in ee_data.columns:
                    ...
                    score_features[feature] = value
                elif feature == 'employment_years' and 'employment_years_score' in ee_data.columns:
                    # Use transformed feature instead
                    value = ee_data['employment_years_score'].iloc[0]
                    score_features[feature] = value
                else:
                    logger.warning(f"Feature {feature} not found in education/employment data")
                    score_features[feature] = 0.0
    ...
    def save_model(self, filepath: str = None) -> None:
        ...
        if filepath is None:
            filepath = os.path.join(self.model_dir, 'education_employment_scorer.joblib')
...
class AlternativeDataScoreAggregator:
    ...
    def _init_default_scorers(self) -> None:
        """Initialize default scorers"""
        self.register_scorer('digital_footprint', DigitalFootprintScorer(self.config.get('digital_footprint', {})))
        self.register_scorer('transaction', TransactionDataScorer(self.config.get('transaction', {})))
        self.register_scorer('utility_payment', UtilityPaymentScorer(self.config.get('utility_payment', {})))
        self.register_scorer('education_employment', EducationEmploymentScorer(self.config.get('education_employment', {})))
    ...
    def aggregate_score(self, individual_scores: Dict[str, float] = None, data: pd.DataFrame = None) -> Tuple[float, Dict[str, float]]:
        """
    ...
    def generate_score_report(self, aggregate_score: float, individual_scores: Dict[str, float]) -> Dict[str, Any]:
        """
