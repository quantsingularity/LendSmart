...
try:
    ...
    validation_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'validation_results')
    ...
    def validate_alternative_data_sources():
        ...
        try:
            ...
            assert len(data_manager.data_sources) >= 4, "Expected at least 4 default data sources"
    ...
    def validate_alternative_data_scoring():
        ...
        try:
            ...
            # Verify score ranges
            assert 0 <= aggregate_score <= 100, f"Aggregate score {aggregate_score} outside valid range 0-100"
            
            for source, score in individual_scores.items():
                assert 0 <= score <= 100, f"Score for {source} ({score}) outside valid range 0-100"
    ...
    def validate_enhanced_ml_models():
        ...
        try:
            ...
            assert all(0 <= p <= 1 for p in y_pred), "Predictions outside valid range 0-1"
            
            # Test credit score calculation
            credit_score = model.calculate_credit_score(y_pred[0])
            assert 300 <= credit_score <= 850, f"Credit score {credit_score} outside valid range 300-850"
    ...
    def validate_compliance_framework():
        ...
        try:
            ...
            assert len(checks) >= 5, f"Expected at least 5 compliance checks, got {len(checks)}"
            ...
            assert len(results) == len(checks), "Number of results doesn't match number of checks"
            ...
            assert os.path.exists(notice_path), "Adverse action notice was not generated"
    ...
    def validate_integration():
        ...
        try:
            ...
            assert 'X_traditional' in training_data, "Missing traditional data in training data"
            assert 'X_alternative' in training_data, "Missing alternative data in training data"
            ...
            assert 'traditional_score' in results, "Missing traditional score in results"
            assert 'alternative_data_score' in results, "Missing alternative data score in results"
            ...
            # Verify score ranges
            assert 0 <= results['traditional_score'] <= 100, "Traditional score outside valid range"
            assert 0 <= results['alternative_data_score'] <= 100, "Alternative score outside valid range"
            assert 300 <= results['enhanced_score'] <= 850, "Enhanced score outside valid range"
            
            # Verify decision is one of the expected values
            assert results['decision'] in ['Approved', 'Conditionally Approved', 'Manual Review Required', 'Declined'], f"Unexpected decision: {results['decision']}"
