...
const getLoanDetailsById = async (loanId) => {
  ...
  const placeholderLoans = [
    { id: '1', amount: 1500, interestRate: 8.5, term: 12, purpose: 'Debt Consolidation', creditScoreRange: '650-700', status: 'Available', borrower: '0x123...abc', description: 'Looking to consolidate high-interest credit card debt into a single, manageable loan.', collateral: 'None', fundedAmount: 0 },
    { id: '2', amount: 500, interestRate: 12.0, term: 6, purpose: 'Small Business', creditScoreRange: '600-650', status: 'Available', borrower: '0x456...def', description: 'Need short-term funding for inventory purchase for my online store.', collateral: 'None', fundedAmount: 100 },
    { id: '3', amount: 3000, interestRate: 7.0, term: 24, purpose: 'Home Improvement', creditScoreRange: '700+', status: 'Available', borrower: '0x789...ghi', description: 'Funding needed for kitchen renovation project. Stable income and good credit history.', collateral: 'None', fundedAmount: 0 },
    { id: '4', amount: 1000, interestRate: 9.0, term: 9, purpose: 'Education', creditScoreRange: '680-720', status: 'Funded', borrower: '0xabc...123', description: 'Loan to cover costs for a professional certification course.', collateral: 'None', fundedAmount: 1000 },
  ];
...
const LoanDetailsScreen = ({ route, navigation }) => {
  ...
  const handleFundLoan = async () => {
    if (!isConnected) {
      Alert.alert('Connect Wallet', 'Please connect your wallet to fund a loan.', [
        { text: 'Cancel' },
    ...
    if (isNaN(amountToFund) || amountToFund <= 0) {
      Alert.alert('Invalid Amount', 'Please enter a valid positive amount to fund.');
      return;
    }
    if (amountToFund > (loan.amount - (loan.fundedAmount || 0))) {
        Alert.alert('Invalid Amount', `You can fund a maximum of $${(loan.amount - (loan.fundedAmount || 0)).toFixed(2)}.`);
        return;
    }
    ...
    try {
      ...
      console.log(`Funding loan ${loanId} with $${amountToFund} from address ${address}`);
      // Example: await blockchainService.fundLoan(loanId, amountToFund);
      await new Promise(resolve => setTimeout(resolve, 2000)); // Simulate transaction

      Alert.alert('Funding Successful', `You have successfully funded $${amountToFund} for loan ${loanId}.`);
      // Optionally navigate back or refresh data
      navigation.goBack();

    } catch (err) {
      console.error('Funding failed:', err);
      Alert.alert('Funding Failed', err.message || 'An error occurred while funding the loan.');
    } finally {
      setIsFunding(false);
    }
  };
  ...
  if (error || !loan) {
    return <View style={styles.errorContainer}><Text style={styles.errorText}>{error || 'Loan details not available.'}</Text></View>;
  }
      ...
      <Card style={styles.card}>
        ...
        <Card.Content>
          <List.Item
            ...
            left={() => <List.Icon icon="percent-outline" color={theme.colors.primary} />}
          />
          <List.Item
            title="Term"
            description={`${loan.term} Months`}
            left={() => <List.Icon icon="calendar-clock-outline" color={theme.colors.primary} />}
          />
          <List.Item
            title="Credit Score Range"
            description={loan.creditScoreRange}
            left={() => <List.Icon icon="credit-card-scan-outline" color={theme.colors.primary} />}
          />
          <List.Item
            title="Borrower"
            ...
            left={() => <List.Icon icon="account-outline" color={theme.colors.primary} />}
      ...
      <Card style={styles.card}>
        <Card.Content>
          ...
          <List.Item
            ...
            left={() => <List.Icon icon={loan.status === 'Available' ? 'check-circle-outline' : 'information-outline'} color={loan.status === 'Available' ? theme.colors.success : theme.colors.disabled} />}
          />
          <List.Item
            title="Amount Funded"
            description={`$${(loan.fundedAmount || 0).toLocaleString()} / $${loan.amount.toLocaleString()}`}
            left={() => <List.Icon icon="cash-multiple" color={theme.colors.primary} />}
          />
          {isFundable && (
            <View style={styles.fundingSection}>
              ...
              {!isConnected && <Text style={styles.connectPrompt}>You need to connect your wallet first.</Text>}
            </View>
          )}
        </Card.Content>
      </Card>
