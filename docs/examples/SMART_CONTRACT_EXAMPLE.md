...
class LendSmartContractClient {
    constructor() {
        ...
        this.tokenContract = new ethers.Contract(process.env.TOKEN_ADDRESS, ERC20_ABI, this.wallet);
    }

    // Request a new loan
    async requestLoan(principal, interestRate, durationDays, purpose, isCollateralized = false) {
    ...
    async listenToLoanEvents() {
        ...
        this.loanContract.on('LoanFunded', (loanId, lender, fundedTime, event) => {
            ...
            console.log('Funded Time:', new Date(fundedTime.toNumber() * 1000).toISOString());
        });
