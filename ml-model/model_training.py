...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'model_training.log')),
...
def load_data():
    ...
    try:
        ...
        logger.info(f"Loaded {len(borrowers_df)} borrower records and {len(transactions_df)} transaction records")
        return borrowers_df, transactions_df
...
def create_synthetic_data(n_borrowers=1000, n_transactions=5000):
    ...
    """
    logger.info(f"Generating synthetic data with {n_borrowers} borrowers and {n_transactions} transactions")
    np.random.seed(42)
    ...
    borrowers_df = pd.DataFrame({
        ...
        'home_ownership': np.random.choice(['RENT', 'OWN', 'MORTGAGE'], n_borrowers),
        'education': np.random.choice(['High School', 'Bachelor', 'Master', 'PhD'], n_borrowers),
        'marital_status': np.random.choice(['Single', 'Married', 'Divorced'], n_borrowers),
        'num_dependents': np.random.randint(0, 5, n_borrowers)
    })
    
    # Calculate debt-to-income ratio
    borrowers_df['debt_to_income'] = (borrowers_df['existing_debt'] / borrowers_df['income']).clip(0, 1)
    ...
    default_prob = (
        ...
        -0.2 * (temp_df['credit_score'] / 100) +  # Higher credit score reduces default risk
        0.15 * (temp_df['loan_amount'] / 10000) +  # Higher loan amount increases default risk
        -0.1 * temp_df['employment_years'] +  # Longer employment reduces default risk
        0.3 * temp_df['debt_to_income'] +  # Higher debt-to-income increases default risk
        0.1 * (temp_df['interest_rate'] / 10) +  # Higher interest rate increases default risk
    ...
    transactions_df = pd.DataFrame({
        ...
        'interest_rate': interest_rates,
        'loan_term': loan_terms,
        'loan_purpose': np.random.choice(['Debt Consolidation', 'Home Improvement', 'Business', 'Education', 'Other'], n_transactions),
        'default': default
    })
...
def feature_engineering(borrowers_df, transactions_df):
    ...
    try:
        ...
        merged_df['payment_to_income'] = (merged_df['loan_amount'] / merged_df['loan_term']) / merged_df['income']
        
        # Total loan burden
        merged_df['total_loan_burden'] = merged_df['loan_amount'] + merged_df['existing_debt']
        
        # Loan amount to income ratio
        merged_df['loan_to_income'] = merged_df['loan_amount'] / merged_df['income']
        
        # Interest burden
        merged_df['interest_burden'] = merged_df['loan_amount'] * (merged_df['interest_rate'] / 100) * (merged_df['loan_term'] / 12)
        ...
        try:
            ...
            if metrics["roc_auc"] > best_roc_auc:
                ...
                if hasattr(current_model, 'feature_importances_'):
                    # Load feature names
                    try:
                        with open(os.path.join(SAVED_MODELS_DIR, "feature_names.txt"), "r") as f:
                        ...
                        if len(importances) != len(feature_names):
                            logger.warning(f"Feature importance length ({len(importances)}) doesn't match feature names length ({len(feature_names)})")
